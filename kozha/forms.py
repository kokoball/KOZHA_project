...
class RegistrationForm(FlaskForm):
    username = StringField('닉네임', validators=[
                           DataRequired(), Length(min=2, max=20)])
    email = StringField('이메일', validators=[DataRequired(), Email()])
    password = PasswordField('비밀번호(Password)', validators=[DataRequired()])
    confirm_password = PasswordField('비밀번호확인(Confirm Password)', validators=[
...
class UpdateAccountForm(FlaskForm):
    username = StringField('닉네임', validators=[
                           DataRequired(), Length(min=2, max=20)])
    email = StringField('이메일', validators=[DataRequired(), Email()])
    picture = FileField('프로필사진', validators=[FileAllowed(['png', 'jpg'])])
